...
https://github.com/user-attachments/assets/syncmind-demo.mp4
...
The URL is **auto-detected** from `.env.local`, `.mcp.json`, or defaults to `http://localhost:3000`. If the server isn't running, the CLI tells you how to start it.
...
# Or with node
SYNCMIND_URL=http://localhost:3000 node /path/to/syncmind/mcp-server/index.js
```
...
- **Memory Browser** — Search, filter by type/source/scope, freshness bars, confidence badges (validated/auto/speculative), stale indicators, scope tags
...
- **Docs (3 tabs)** — MCP setup, CLI reference, REST API — all in-dashboard
...
├── mcp-server/
│   ├── index.js                 # MCP server v2 (4 tools, freshness, dedup)
│   ├── cli.js                   # SyncMind CLI v2 (9 commands, auto-capture)
...
Add these sync rules in your PowerSync dashboard:
